...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Directory containing multiple saved PEFT checkpoints."
    )
    parser.add_argument(
        "--test_data_path",
        type=str,
    ...
    parser.add_argument(
        "--output_file",
        type=str,
        default="learning_curve.json",
        help="Path to save the learning curve data as JSON."
    )
    ...
    for i in range(100, 1300, 100):
        checkpoint = f"{args.checkpoints_dir}/checkpoint-{i}"
         # Load LoRA
        peft_model = PeftModel.from_pretrained(model, checkpoint)
        peft_model.eval()
        ppl = perplexity(peft_model, tokenizer, data)
        mean_ppl = ppl["mean_perplexity"]
        results.append({
            "checkpoint": checkpoint,
            "mean_perplexity": mean_ppl
        })
        print(f"Checkpoint {checkpoint}: Mean perplexity = {mean_ppl}")
