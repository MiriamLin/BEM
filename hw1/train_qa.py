...
def save_learning_curve_metrics(results, output_dir, file_name: str = "learning_curve_data.json"):
    with open(os.path.join(output_dir, file_name), "w") as f:
        json.dump(results, f, indent=4) 
...
def main():
    ...
    def prepare_train_features(examples):
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            allAnswers = [{'text': [ans['text']], 'answer_start': [ans['start']]} for ans in examples['answer']]
            answers = allAnswers[sample_index]            
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                outputs = model(**batch)
                loss = outputs.loss
                cumulative_loss += loss.detach().float().item()
                # We keep track of the loss at each epoch
                if args.with_tracking:
                    total_loss += loss.detach().float()
                start_logits = outputs.start_logits
                end_logits = outputs.end_logits
                predictions = (start_logits.argmax(dim=-1), end_logits.argmax(dim=-1))
                labels = batch["start_positions"], batch["end_positions"]
                em_score = ((predictions[0] == labels[0]) & (predictions[1] == labels[1])).float().mean().item()
                cumulative_em += em_score
            ...
            if steps % 1000 == 0:
                avg_loss = cumulative_loss / 1000
                avg_em = cumulative_em / 1000
    ...
    save_learning_curve_metrics(learning_curve_data, args.output_dir)
